...
describe('QRTypeSelector', () => {
  ...
  it('renders with correct label and options', () => {
    ...
    expect(screen.getByRole('option', { name: 'Text/URL' })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: 'Contact Card (vCard)' })).toBeInTheDocument();
  });
  ...
  it('renders with text selected by default', () => {
    ...
    const textOption = screen.getByRole('option', { name: 'Text/URL' }) as HTMLOptionElement;
    const vcardOption = screen.getByRole('option', { name: 'Contact Card (vCard)' }) as HTMLOptionElement;
