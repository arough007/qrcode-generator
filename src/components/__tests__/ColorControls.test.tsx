...
describe('ColorControls', () => {
  ...
  it('renders all color controls with correct labels', () => {
    render(<ColorControls colors={defaultColors} onColorChange={mockOnColorChange} />);
    
    expect(screen.getByLabelText('QR Code Color:')).toBeInTheDocument();
    expect(screen.getByLabelText('Background Color:')).toBeInTheDocument();
    expect(screen.getByLabelText('Use transparent background')).toBeInTheDocument();
  });
  ...
  it('displays current color values', () => {
    ...
    render(<ColorControls colors={customColors} onColorChange={mockOnColorChange} />);
    
    const foregroundInput = screen.getByLabelText('QR Code Color:') as HTMLInputElement;
  ...
  it('calls onColorChange when foreground color changes', () => {
    ...
    expect(mockOnColorChange).toHaveBeenCalledWith('foregroundColor', '#ff0000');
  });
  ...
  it('calls onColorChange when background color changes', () => {
    ...
    expect(mockOnColorChange).toHaveBeenCalledWith('backgroundColor', '#00ff00');
  });
  ...
  it('calls onColorChange when transparent background checkbox is toggled', () => {
    ...
    expect(mockOnColorChange).toHaveBeenCalledWith('transparentBackground', true);
  });
  ...
  it('disables background color input when transparent background is enabled', () => {
    ...
    render(<ColorControls colors={transparentColors} onColorChange={mockOnColorChange} />);
  ...
  it('shows checkbox as checked when transparentBackground is true', () => {
    ...
    const checkbox = screen.getByLabelText('Use transparent background') as HTMLInputElement;
